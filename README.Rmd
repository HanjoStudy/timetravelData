...
The searching functionality uses a very simple interface through the `query_text` function. Lets imagine we need to look for whether the word "cotton" is contained in the text:
...
You can also look up how many _hits_ were seen in the database:

```{r, eval = TRUE}
total_hits <- query_hits("cotton")
```

From the example, we can see that the word "cotton" was observed on `r length(total_hits)` pages.
